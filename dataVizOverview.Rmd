...
## Pre-writing Your Visualization
...
- GEOM: Line linear prediction (x = `total_bill`, y = `tip`)
...
- Often we need to use a visualization to think about a very complex problem 
and to invite exploration and test multiple hypotheses
...
p1 <- ggplot(tips, aes(y = tip, x = total_bill, color = day, shape = time)) +
  geom_point() + theme_bw() + theme(legend.position = c(.41, .85), 
...
```{r colorapp}
 inputPanel(
   selectInput("type", label = "Palette Type",
    ...
    numericInput("pal", label = "Palette Selection", 1,
      min = 1, max = 9, step = 1)
   )
 renderPlot({
