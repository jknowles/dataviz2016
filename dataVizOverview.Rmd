...
- Using this "grammar" we can begin to compose arguments
- Key to doing this is to think about the dimensionality of the data and 
the complexity of the visualization
- Dimensions limit how many data elements we can encode into one graphic
- More dimensions = more data, but does not necessarily = more understanding
- Aesthetics are one method of encoding data, layers and facets are another
- Sometimes we may assign the same data element to multiple dimensions for 
emphasis

## The third dimension

- Instead of thinking about a 3d plot, consider faceting
- Facets are a technique for repeating plot elements across multiple 
types of data

```{r facet}
library(gridExtra)

p1 <- ggplot(tips, aes(y = tip, x = total_bill, color = day)) + geom_point() + 
  theme_bw() + theme(legend.position = c(.15, .75))


p2 <- ggplot(tips, aes(y = tip, x = total_bill)) + geom_point() + 
...
ggplot(plotdf, aes(x = x, y = y)) + geom_bar(stat = 'identity') + 
  scale_y_continuous(label = percent) + theme_classic(base_size = 14)
...
ggplot(plotdf, aes(x = reorder(x, y), y = y, fill = col)) + geom_bar(stat = 'identity') + 
  ...
  scale_fill_brewer("Benchmark", type = "qual", palette = 2) + 
  geom_hline(yintercept = 0.7) + 
  geom_text(label = paste0(round(plotdf$y*100, 0), "%"), size = 3.5, vjust = -1) +
  annotate(geom = "text", x = 5, y = 0.75, label = "Performance benchmark") + 
...
#   selectInput("pal", label = "Palette Selection",
...
shinyApp(
 ...
 selectInput("type", label = "Palette Type",
  ...
  server = function(input, output) {
  # p1 <- ggplot(tips, aes(x = total_bill, y = tip, color = day)) + 
  # geom_point() +
  # labs(title="Restaurant Tipping Patterns") + 
  # theme_classic(base_size = 16)
  # colPal <- reactive({
  ...
  #   })
  output$scalePlot <- renderPlot(
  {ggplot(tips, aes(x = total_bill, y = tip, color = day), environment = environment()) + 
  geom_point() +   labs(title="Restaurant Tipping Patterns") + 
  theme_classic(base_size = 16) +  
    scale_color_brewer(type = input$type, palette = input$pal)}
  , width=600 , height=500)
